@props(['name' => null, 'helptext' => null, 'label' => null, 'value' => '1', 'size' => 'full', 'checked' => false])
...
<div class="@if ($size == 'full') w-full @elseif ($size == 'half') w-full md:w-1/2 md:px-1 @endif mb-4">
    <label class="inline-flex items-center cursor-pointer label">
        <input type="checkbox" id="{{ $name }}" name="{{ $name }}" value="{{ $value }}" class="checkbox checkbox-primary @error($name) border-error @else border-primary @enderror" @if ($checked) checked @endif>
